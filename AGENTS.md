...
│   ├── 全国评分数据8月-美团.xlsx
│   ├── 全国评分数据8月-闪购.xlsx
│   └── 有公式/                         # 主流程只扫一层、不进这里；美团「消息回复率」另见 3.2
└── 基础数据/<期>.xlsx                  # 可选，只用来补 门店ID / 门店类型
```

- 文件名含「美团」/「闪购」即被识别；`~$` 开头的 Excel 锁文件自动跳过。
- **有公式子目录（3.2）**：`_rating_files()` 仍然只扫一层，美团消息回复率由 `_formula_file()` 单独定位读取。
...
- **美团「消息回复率」百分比没有列，靠聚合**：美团门店维度只有「消息回复率得分（10%）」，`mt_reply` 取自（有公式）文件的明细表，口径见 3.2 与「已知特例」11。
...
### 3.2 美团消息回复率（`_read_mt_reply`）

`评分数据\有公式\全国评分数据*-美团（有公式）.xlsx` 的 `全国本期` / `全国上期` 是**门店 × 日期**明细（`全国本期` 100346 行 / `全国上期` 99889 行），第 18 列为「消息回复率」（小数）。

| 项 | 取值 |
| --- | --- |
| 门店名 | 第 5 列（F），形如 `喜茶(佛山顺德大良东乐路店)`，需 `_norm_store()` 去掉外壳并统一全/半角括号 |
| 门店 ID | 第 6 列（G），名称匹配不上时用底表 `门店ID` 兜底 |
| 取值 | 对门店**全部日期取平均**（`全国本期` = 8 月、`全国上期` = 7 月） |

- 为什么取全部日期平均：该文件自己的公式就是这么算的（`华南门店维度` 与 `全国本期` 同列均值一致，已核对 191/191 家）；窗口不是「本周」。
...
| 指标 | 公式 |
| --- | --- |
| `mt_quality` / `sg_quality` 商品质量分 | 满意度×30% + 包装满意度×10% + 复购率指标得分×20% + 食品安全负反馈率×20%（**满分 4 分**） |
| `mt_service` / `sg_service` 服务体验分 | 消息回复率×10% + 服务负反馈率×10%（**满分 1 分**） |

- 闪购的「满意度」取 `sg_taste_sat` 口味满意度；美团取 `mt_goods_sat`。
- **按权重直接求和，不要除以权重合计**：`评分 = 商品质量分 + 服务体验分`（满分 5 分）。商品质量分本身满分只有 4 分、服务体验分只有 1 分，这是平台口径，不是异常。
...
- **门店全景排名（视图2）表格列序**：`#`（名次）→ 督导 → 城市 → 门店 → 勾选的指标列；改列序时 `storeDetailRows()` 的 `<th>` 与 `<td>` 两处要同步改。
...
11. **美团 `mt_reply` 的口径**：见 3.2，按门店对（有公式）明细**全部日期取平均**，不是本周值；门店名对不上时用门店 ID 兜底，且只补美团门店维度里出现过的门店（例：`福州仓山万达店` 只在闪购/明细里出现 → 不给 `mt_reply`，显示 `—`）。
12. **`_rating_files()` 依然跳过 `有公式/` 子目录**：有公式文件单独由 `_formula_file()` 定位，别把它并进主扫描，否则同一门店会被两份文件互相覆盖。
